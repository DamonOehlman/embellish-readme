...
class Badges {
  ...
  static nodeico(packageData /*: Package */, basePath /*: string */, reporter /*: ?Reporter */) /*: Promise<string> */ {
    reporter && reporter('nodei.co:    !{check|green}')
  ...
  static async travis(packageData /*: Package */, basePath /*: string */, reporter /*: ?Reporter */) /*: Promise<string> */ {
    const repository = packageData.getRepositry();
    const haveTravisFile = await isFilePresent(join(basePath, '.travis.yml'));
    if (!haveTravisFile) {
      reporter && reporter('travis:      !{0x2717|yellow} not configured (see: https://travis-ci.org/)')
      return '';
    }

    reporter && reporter('travis:      !{check|green}');
  ...
  static bithound(packageData /*: Package */, basePath /*: string */, reporter /*: ?Reporter */) /*: Promise<string> */ {
    reporter && reporter('bithound:    !{0x2717|red} service retired :(');
    return '';
  }

  static stability(packageData /*: Package */, basePath /*: string */, reporter /*: ?Reporter */) /*: Promise<string> */ {
    const stability = packageData.stability;
    const stabilityColor = stability && stabilityColors.get(stability);
    if (!stabilityColor) {
      reporter && reporter('stability:   !{0x2717|yellow} not specified (or invalid stability) in package.json')
      return '';
    }

    reporter && reporter(`stability:   !{check|green} (stability = ${stability})`);
    return Promise.resolve([
      '[',
      `![${stability}]`,
  ...
  static async codeClimateMaintainability(packageData /*: Package */, basePath /*: string */, reporter /*: ?Reporter */) /*: Promise<string> */ {
    const repository = packageData.getRepositry();
    const host = repository && repository.host.split('.')[0];

    const codeClimateId = repository && await fetchCodeClimateId(repository.path);
    if (!codeClimateId) {
      reporter && reporter('codeclimate: !{0x2717|yellow} not configured (see: https://codeclimate.com)')
      return '';
    }

    reporter && reporter(`codeclimate !{check|green} (badge id = ${codeClimateId})`);
